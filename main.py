...
@app.route("/")
def index():
    # UI is "ready" if the library is installed; user can paste a key in-app even without env var.
    gemini_ready = bool(_GEMINI_AVAILABLE)
    gemini_env_key = bool(os.environ.get("GEMINI_API_KEY"))
    return render_template("index.html", gemini_ready=gemini_ready, gemini_env_key=gemini_env_key)
